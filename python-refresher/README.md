...
# assign them to variables, provide as arguments, or return.
def create_adder(x):
    """"""
    def adder(y):
        return x + y
...
# As well as some higher-order functions (`map`, `filter`)
list(map(add_10, [1, 2, 3]))          # => [11, 12, 13]

# However, Python encourages to use comprehension (if possible)
# instead (see below).
```
